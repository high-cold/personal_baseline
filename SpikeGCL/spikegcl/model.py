...
from torch_geometric.utils import dropout_edge, mask_feature, to_torch_csr_tensor
...
class SpikeGCL(torch.nn.Module):
    def __init__(
        ...
        self.lin = torch.nn.Linear(hidden_channels, hidden_channels, bias=False)  # out -> hide
    ...
    def encode(self, x, edge_index, edge_weight=None):
        ...
        edge_index = to_torch_csr_tensor(edge_index, size=x.size(0))
        xs = []
    ...
    def forward(self, x, edge_index, edge_weight=None):
        ...
        s = torch.stack(s1, dim=0)
        # print(f"{s.sum() * 3.6 * 1e-9 + 4.7 * s.sum()  * 1e-9} mJ")
        z1 = self.decode(s1)
        z2 = self.decode(s2)
        return z1, z2
