...
# dataset = Amazon(root_path + '/data', name='computers', transform=transform)
dataset = Coauthor(root_path + '/data', name='CS', transform=transform)
...
def main():
    model = SGC_LP(dataset.num_features, 32, 64).to(device)
    aucs, aps = [], []
    for i in range(2):
